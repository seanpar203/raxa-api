...
type User struct {
	...
	HasVerifiedPhone    bool        `boil:"has_verified_phone" json:"has_verified_phone" toml:"has_verified_phone" yaml:"has_verified_phone"`
...
var UserTableColumns = struct {
	...
	HasVerifiedPhone:    "users.has_verified_phone",
...
var UserWhere = struct {
	...
	HasVerifiedPhone    whereHelperbool
	...
	HasVerifiedPhone:    whereHelperbool{field: "\"users\".\"has_verified_phone\""},
...
// Otps retrieves all the otp's Otps with an executor.
func (o *User) Otps(mods ...qm.QueryMod) otpQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// LoadOtps allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOtps(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on otp")
	}
	if err = results.Err(); err != nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Otps = append(local.R.Otps, foreign)
				if foreign.R == nil {
					foreign.R = &otpR{}
				}
				foreign.R.User = local
...
func (o *User) AddAccessTokensG(ctx context.Context, insert bool, related ...*AccessToken) error {
	return o.AddAccessTokens(ctx, boil.GetContextDB(), insert, related...)
}
...
// AddOtpsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Otps.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddOtpsG(ctx context.Context, insert bool, related ...*Otp) error {
	return o.AddOtps(ctx, boil.GetContextDB(), insert, related...)
}

// AddOtps adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Otps.
// Sets related.R.User appropriately.
func (o *User) AddOtps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Otp) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Otps = append(o.R.Otps, related...)
	}
...
// AddRefreshTokensG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RefreshTokens.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddRefreshTokensG(ctx context.Context, insert bool, related ...*RefreshToken) error {
	return o.AddRefreshTokens(ctx, boil.GetContextDB(), insert, related...)
}
